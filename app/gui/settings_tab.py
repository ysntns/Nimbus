...
class SettingsTab:
    ...
    def _create_widgets(self):
        ...
        self.backup_location_entry = ctk.CTkEntry(
            location_frame,
            placeholder_text="Select default backup location..."
        )
        ...
        self.default_incremental = ctk.BooleanVar(
            value=self.config.get('default_incremental', True)
        )
        ctk.CTkCheckBox(
            backup_frame,
        ...
        self.default_encrypt = ctk.BooleanVar(
            value=self.config.get('default_encrypt', True)
        )
        ctk.CTkCheckBox(
            backup_frame,
            text="Enable Encryption by Default",
            variable=self.default_encrypt,
            font=ctk.CTkFont(size=13)
        ).grid(row=1, column=0, padx=15, pady=10, sticky="w")

        self.default_compress = ctk.BooleanVar(
            value=self.config.get('default_compress', False)
        )
        ctk.CTkCheckBox(
            backup_frame,
            text="Enable Compression by Default",
            variable=self.default_compress,
            font=ctk.CTkFont(size=13)
        ).grid(row=2, column=0, padx=15, pady=10, sticky="w")

        self.default_verify = ctk.BooleanVar(
            value=self.config.get('default_verify', True)
        )
        ctk.CTkCheckBox(
            backup_frame,
            text="Enable Verification by Default",
            variable=self.default_verify,
            font=ctk.CTkFont(size=13)
        ...
        self.notify_success = ctk.BooleanVar(
            value=self.config.get('notify_success', True)
        )
        ctk.CTkCheckBox(
            notif_frame,
            text="Notify on Successful Backup",
            variable=self.notify_success,
            font=ctk.CTkFont(size=13)
        ).grid(row=0, column=0, padx=15, pady=10, sticky="w")

        self.notify_error = ctk.BooleanVar(
            value=self.config.get('notify_error', True)
        )
        ctk.CTkCheckBox(
            notif_frame,
            text="Notify on Backup Error",
            variable=self.notify_error,
            font=ctk.CTkFont(size=13)
        ).grid(row=1, column=0, padx=15, pady=10, sticky="w")
        ...
        self.max_threads_var = ctk.StringVar(
            value=str(self.config.get('max_threads', 4))
        )
